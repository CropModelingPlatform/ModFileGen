...
def common_tempoparv6(modelDictionary, sticsv):
    fileContent = ""
    ...
    if sticsv == "v9":
        ...
        fileContent += format_stics_data_v6(DT, "codepluiepoquet")
        fileContent += format_stics_data_v6(DT, "nbjoursrrversirrig")
        fileContent += format_stics_data_v6(DT, "codecalferti") # move
        fileContent += format_stics_data_v6(DT, "ratiolN", 5)
        fileContent += format_stics_data_v6(DT, "dosimxN", 5)
        fileContent += format_stics_data_v6(DT, "codetesthumN") # move
        fileContent += format_stics_data_v6(DT, "codeNmindec")
        fileContent += format_stics_data_v6(DT, "rapNmindec", 5)
        fileContent += format_stics_data_v6(DT, "fNmindecmin", 5)
        fileContent += format_stics_data_v6(DT, "codetrosee")
        ...
        fileContent += format_stics_data_v6(DT, "engrais_pature")
        fileContent += format_stics_data_v6(DT, "coef_calcul_doseN")
        #fileContent += format_stics_data_v6(DT, "code_CsurNsol_dynamic") # To add
        fileContent += "code_CsurNsol_dynamic\n"
        fileContent += "2\n"
        #fileContent += format_stics_data_v6(DT, "humirac") # add
        fileContent += "humirac\n"
        fileContent += "1\n"
        #fileContent += format_stics_data_v6(DT, "code_ISOP") # add
        fileContent += "code_ISOP\n"
        fileContent += "2\n"
        #fileContent += format_stics_data_v6(DT, "code_pct_legume") # add
        fileContent += "code_pct_legume\n"
        fileContent += "2\n"
        #fileContent += format_stics_data_v6(DT, "pct_legum") # add
        fileContent += "pct_legum\n"
        fileContent += "0.5\n"
...
def common_tempopar(ModelDictionary, vstics):
    ModelDictionary_Connection = sqlite3.connect(ModelDictionary)
    ...
    if vstics == "v9":
        ...
        fileContent += format_stics_data(DT, "codesymbiose")
        fileContent += format_stics_data(DT, "codefxn")
        fileContent += "tmin_mineralisation\n"   # To ADD
        fileContent += "0.0\n"
        fileContent += format_stics_data(DT, "FTEMh")
        fileContent += format_stics_data(DT, "FTEMha")
        fileContent += format_stics_data(DT, "TREFh")
...
def process_chunk(chunk, mi, md, tpv6,tppar, directoryPath,pltfolder, rap, var, prof, dt, vstics):
    dataframes = []
    ...
    for i, row in enumerate(chunk):
        ...
        try:
            ...
            if soilid not in soiltable:
                ...
                r1 = paramsolconverter.export(simPath, ModelDictionary_Connection, MasterInput_Connection, usmdir, vstics)           
                stationconverter = sticsstationconverter.SticsStationConverter()
                r2 = stationconverter.export(simPath, ModelDictionary_Connection, MasterInput_Connection, rap, var, prof, usmdir, vstics)         
            ...
            if iniid not in initable:            
                ...
                r = ficiniconverter.export(simPath, ModelDictionary_Connection, MasterInput_Connection, usmdir, vstics)
                initable[iniid] = r
            else:
            ...
            if tecid not in tectable:  
                ...
                r = fictec1converter.export(simPath, ModelDictionary_Connection, MasterInput_Connection, usmdir, vstics)
                tectable[tecid] = r
                if isinstance(r, list) : nbplt = len(r)
            else:
                if isinstance(tectable[tecid], list):
            ...
            subprocess.run(["bash", bs, usmdir, directoryPath, str(dt), vstics, str(nbplt)])
            ...
            if nbplt == 1:
                ...
                mod_r_p = os.path.join(directoryPath, f"mod_rapport_{str(row['idsim'])}_P.sti") 
                mod_r_a = os.path.join(directoryPath, f"mod_rapport_{str(row['idsim'])}_A.sti") 
                # check if the file exists
                if not os.path.exists(mod_r_p):
                    print(f"Error: {mod_r_p} does not exist")
                    return None
                if not os.path.exists(mod_r_a):
                    print(f"Error: {mod_r_a} does not exist")
                    return None
                df_p = create_df_summary(mod_r_p)
                df_a = create_df_summary(mod_r_a)
...
def main():
    ...
    vstics = GlobalVariables["stics"]
    export(mi, md)
    tppar = common_tempopar(md, vstics)
    tpv6 = common_tempoparv6(md, vstics)
    rap = common_rap()
    var = common_var()
    prof = common_prof()
    ...
    try:
        ...
        with Pool(processes=nthreads) as pool:
            ...
            processed_data_chunks = pool.starmap(process_chunk,[(chunk,mi, md, tpv6,tppar,directoryPath,pltfolder, rap, var, prof, dt, vstics) for chunk in chunks])  
